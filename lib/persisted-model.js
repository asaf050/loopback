...
module.exports = function (registry) {
  ...
  PersistedModel.upsert = PersistedModel.updateOrCreate = PersistedModel.patchOrCreate = function upsert (
    data,
    callback
  ) {
  ...
  PersistedModel.upsertWithWhere = PersistedModel.patchOrCreateWithWhere = function upsertWithWhere (
    where,
    data,
    callback
  ) {
  ...
  PersistedModel.prototype.updateAttribute = function updateAttribute (
    name,
    value,
    callback
  ) {
  ...
  PersistedModel.prototype.updateAttributes = PersistedModel.prototype.patchAttributes = function updateAttributes (
    data,
    cb
  ) {
  ...
  PersistedModel.prototype.replaceAttributes = function replaceAttributes (
    data,
    cb
  ) {
  ...
  PersistedModel.setupRemoting = function () {
    ...
    var upsertOptions = {
      ...
      description:
        'Patch an existing model instance or insert a new one ' +
    ...
    setRemoting(PersistedModel, 'upsertWithWhere', {
      ...
      description:
        'Update an existing model instance or insert a new one into ' +
    ...
    var updateAttributesOptions = {
      ...
      description:
        'Patch attributes for a model instance and persist it into ' +
        'the data source.',
      accessType: 'WRITE',
      accepts: [
        {
    ...
    if (options.trackChanges || options.enableRemoteReplication) {
      ...
      setRemoting(PersistedModel, 'changes', {
        description:
          'Get the changes to a model since a given checkpoint.' +
  ...
  PersistedModel.replicate = function (since, targetModel, options, callback) {
    ...
    debug(
      'replicating %s since %s to %s since %s',
    ...
    function run (attempt, since) {
      ...
      function next (err, conflicts, cps, updates) {
        ...
        if (finished) return callback(err, conflicts, cps)
        run(attempt + 1, cps)
      }
    }
  }
  ...
  function tryReplicate (sourceModel, targetModel, since, options, callback) {
        ...
        function (notUsed, err) {
          ...
          if (conflicts && err.statusCode == 409) {
            ...
            updates = updates.filter(function (u) {
              return (
                conflicts.filter(function (d) {
                  return d.modelId === u.change.modelId
                }).length === 0
              )
            })
            return cb()
    ...
    function checkpoints () {
      ...
      sourceModel.checkpoint(function (err, source) {
        ...
        targetModel.checkpoint(function (err, target) {
          ...
          debug(
            '\t\t%s for source model %s',
            newSourceCp,
            sourceModel.modelName
          )
          debug(
            '\t\t%s for target model %s',
            newTargetCp,
            targetModel.modelName
          )
          cb()
    ...
    function done (err) {
      ...
      debug(
        '\t\tnew checkpoints: { source: %j, target: %j }',
        newSourceCp,
        newTargetCp
      )

      var conflicts = diff.conflicts.map(function (change) {
        return new Change.Conflict(change.modelId, sourceModel, targetModel)
      })
            ...
            Model.findById(change.modelId, function (err, inst) {
              ...
              if (!inst) {
                return (
                  ...
                  cb(
                    new Error(
                      g.f('Missing data for change: %s', change.modelId)
                    )
                  )
                )
              }
  ...
  function buildLookupOfAffectedModelData (Model, updates, callback) {
    ...
    var affectedIds = updates.map(function (u) {
  ...
  function applyUpdate (
    ...
    Model.updateAll({ id: current.id }, data, options, function (err, result) {
      if (err) return cb(err)
      ...
      switch (count) {
        ...
        case 0:
          debug(
            'UpdateAll detected non-rectified change of %s %j',
            Model.modelName,
            id
          )
          conflicts.push(change)
        ...
        default:
          debug(
            '%s.updateAll modified unexpected number of instances: %j',
            Model.modelName,
            count
          )
          return cb(
  ...
  function applyDelete (Model, id, current, change, conflicts, options, cb) {
    ...
    Model.deleteAll({ id: current.id }, options, function (err, result) {
      if (err) return cb(err)
      ...
      switch (count) {
        ...
        case 0:
          debug(
            'DeleteAll detected non-rectified change of %s %j',
            Model.modelName,
            id
          )
          conflicts.push(change)
        ...
        default:
          debug(
            '%s.deleteAll modified unexpected number of instances: %j',
            Model.modelName,
            count
          )
          return cb(
  ...
  PersistedModel.enableChangeTracking = function () {
    if (PersistedModel.cleanupInterval) {
      clearInterval(PersistedModel.cleanupInterval)
    }
    var Model = this
    ...
    if (runtime.isServer && cleanupInterval > 0) {
      ...
      // cleanup
      PersistedModel.cleanupInterval = setInterval(cleanup, cleanupInterval)
    }
  ...
  function rectifyOnSave (ctx, next) {
    if (ctx.Model.settings.trackChanges) {
      ...
      if (debug.enabled) {
        debug(
          'rectifyOnSave %s -> ' + (id ? 'id %j' : '%s'),
          ctx.Model.modelName,
          id || 'ALL'
        )
        debug(
          'context instance:%j currentInstance:%j where:%j data %j',
          ctx.instance,
          ctx.currentInstance,
          ctx.where,
          ctx.data
      ...
      if (id != null) {
        ...
        console.error(`ID Attribute is null. Could not make rectify on all models, need to make manually or use 'safeUpdateAll'`)
        next()
      }
  ...
  PersistedModel.updateLastChange = function (id, data, cb) {
    ...
    this.findLastChange(id, function (err, inst) {
      ...
      if (!inst) {
        err = new Error(
          g.f('No change record found for %s with id %s', self.modelName, id)
        )
        err.statusCode = 404
        return cb(err)
      }
