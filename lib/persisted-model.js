...
module.exports = function (registry) {
  ...
  function buildLookupOfAffectedModelData (Model, updates, callback) {
    ...
    const affectedIds = updates.map(function (u) {
  ...
  function applyUpdate (
    ...
    Model.updateAll(where, data, options, function (err, result) {
      if (err) return cb(err)
  ...
  function rectifyOnSave (ctx, next) {
    if (ctx.Model.settings.trackChanges) {
      ...
      if (id != null) {
        ...
        reportErrorAndNext()
        console.error(
          `ID Attribute is null. Could not make rectify on all models, need to make manually or use 'safeUpdateAll'`
        )
      }
